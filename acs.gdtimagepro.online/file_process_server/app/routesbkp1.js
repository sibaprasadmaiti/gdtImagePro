...
module.exports = function(app, con)
{
                ...
                if (zipEntry.entryName.includes(".json"))
                {
                    ...
                    if (merchantValid == "")
                    {
                        ...
                        con.query(sql, function(err, result)
                        {
                            ...
                            {
                                merchantValid = true;
                                console.log("mv", merchantValid)
                                currentnumberofrecords = result[0][currentMonth];
                                if (result[0].status == "Advanced") merchantwithImage = true;
                                console.log("Merchant Image", merchantwithImage, result[0].status)
                                ...
                                else s3Image(a, i, inputdata, newcolumns, imagename, a.orderItemId, imagename1, merchantwithImage);
                            }
                        });
                    }
                    else if (merchantValid == false)
                    {
                        console.log(err, "Incorrect Merchant Id");
                    }
                    else
    ...
    // s3Image()
    function s3Image(a, i, inputdata, newcolumns, imagename, orderItemId, imagename1, merchantwithImage)
    {
        console.log("Inside S3 Image")
        ...
        {
            console.log("appending data async",  results[2])
            appenddata(a, i, inputdata, newcolumns, results[0], results[1], results[2])
        });
    }
    ...
    function customImage(data, FinalName, callback)
    {
        ...
        loadImage(process.cwd() + "/zipcontent/" + imagename).then((image) =>
        {
            ...
            //---------text ----------//
            ctx.save();
            var txt = data.customizationData.children[0].children[0].children[1].children[2].children[0].children[0].inputValue;
            ...
            // let file = process.cwd() + "/" + FinalName + ".png";
            // console.log(file)
            const config = {
    ...
    function appenddata(data, i, inputdata, newcolumns, imagepath, imagepath1, imagepath2)
    {
        console.log("Append data", i,imagepath2)
    ...
    {
        console.log("Inside InsertNumberOfRecordsDb")
